...
# sajugen — Claude Code 전용 지침 (공통 규율·성격·Codex 계약은 위 AGENTS.md 가 단일 소스)

사주(명리 메인) + 자미두수(보완) 종합형 PDF 리포트 생성기. 파이프라인·프로젝트 성격 6개조·작업
규율(YOU MUST)·Programmatic checks 는 AGENTS.md 참조 — 여기 중복 기재하지 않는다.
...
- FastAPI(>=0.136) + starlette==1.0.0 + SQLite(주문·검수, WAL) / pydantic + Instructor / LLM = Anthropic 공식 API만

## 실행/테스트 명령
- 전체 테스트(완료 근거는 항상 이것): `./.venv/Scripts/python.exe -m pytest tests/ -q`
...
- 음력 변환 1차 기준 = KASI. 자미 윤달 = 15일 분할법. 자시 기본 = JST_2300(일주 익일 전환 — 코드 배선 완료 2026-07-03).
