...
- **owner**: **Claude Code (신선 세션)** — 운영자 승인 2026-08-17, 사유: Codex 토큰 소진
- **next_reviewer**: **별도 신선 Claude Code 세션 (read-only)** — 구현 세션과 반드시 분리
- **base_commit**: `2b2abb6`
- **rev**: 2 (운영자 승인으로 구현 주체 확정 — §0)

---

## 0. 운영자 승인 기록 (AGENTS.md 기본값 이탈)

AGENTS.md 역할 분리의 기본값은 **Codex 구현 → Claude 교차리뷰**다.
이 태스크는 **Codex 토큰 소진**으로 그 경로를 쓸 수 없어 운영자가 예외를 승인했다.
...
| 승인자 | 운영자 |
| 사유 | Codex 토큰 부재 |
| 구현 | **신선 Claude Code 세션** (이 세션 아님 — 설계자와 분리) |
| 검증 | **또 다른 신선 Claude Code 세션, read-only** |
| 근거 조항 | AGENTS.md "운영자가 Claude 구현을 별도로 승인한 경우만 예외다" |

### 이 예외가 성립하는 이유

역할 분리가 노리는 것은 **벤더 차이가 아니라 컨텍스트 독립성**이다. 신선 세션이면 목적은 충족된다.
게다가 이 패킷은 수용 기준이 **전부 기계적으로 측정 가능**해 리뷰어의 판단력 의존도가 낮다
(기준값 5개 · `width_min_max == 0` · 골든 22건 · `pytest ≥ 1136`).

### 남는 위험과 그 대비

같은 모델이라 **같은 사각을 공유할 수 있다.** 실제로 설계 세션이 이 함정에 빠졌다 —
절입 불일치를 처음 "유파 차이"로 분류했다가 KASI 원본을 보고 뒤집었다.

→ 그 위험은 패킷이 이미 차단한다. **KASI 절기 원본을 기준값으로 못 박아** 두었으므로
"유파 차이 아니냐"로 흐를 여지가 없다. 리뷰어는 §6 표의 값과 일치하는지만 보면 된다.

### 구현 세션이 지켜야 할 경계

- **이 패킷을 쓴 세션이 구현하지 않는다.** 새 세션에서 `이어받아` 로 시작한다.
- **구현 세션이 자기 결과를 검증하지 않는다.** 검증은 별도 세션이 read-only 로 한다.
- Codex 상시 금지(PDF 재생성 · LLM 호출 · commit · push · 배포)는
  **Claude 구현에도 동일 적용**한다. 커밋·푸시는 운영자 승인 사항이다.
...
1. ~~**구현 주체**~~ → **해소(2026-08-17).** Claude Code 신선 세션 구현 + 별도 신선 세션
   read-only 검증. Codex 토큰 부재로 인한 승인된 예외. 상세 §0
2. ~~**대운수 0.57% 변동 수용 여부**~~ → **해소(2026-08-17).**
   `data/orders.sqlite` 실측 결과 `DRAFTED 3 · IN_REVIEW 1 · NORMALIZED 1`,
   **APPROVED 0 · 발송 0** → 소급 영향 없음. 지금이 고칠 수 있는 가장 싼 시점이다.
   다만 done_when 의 "0.57% 변동 명시 보고"는 **유지**한다(수치 자체는 보고 대상)
