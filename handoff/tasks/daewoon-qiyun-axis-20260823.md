...
- **rev**: 3 (2026-08-23 — 리뷰어 독립 재검산에서 **子時 날짜 귀속 결함**(블로커) 발견: §3-1 rev3 정정 블록 + §5 rev3 항목. rev1 워킹트리의
  `compute_qiyun` 날짜 규칙과 손계산 앵커 4건을 그에 맞게 **수정**한다. 나머지 rev2 내용은 유지.)
- **rev 2 이력** (2026-08-23 — rev1 의 §7 정지선 "변동 2% 초과" 는 docs/27 §2 프로브 C(출생 끝점만 진태양시) 에서 나온 **예측 오차**였다.
...
★ **rev3 정정 — 子時의 날짜 귀속 (리뷰어 독립 재검산 2026-08-23, 블로커)**: 時辰 idx 를 23시=子(0) 으로 두면 **날짜도 자시 정책대로
넘겨야** 한다. rev1 구현은 진태양시 23:54 子 를 같은 날짜(12-31)의 idx 0 으로 두고 借位해 大雪 辰(12-07 07:04)→출생을 `23日8辰` 으로
셌지만, 물리 간격은 24일 16.8시간 = **`24日8辰`** 이다(1日=4個月 과소). 절입이 23시대인 달(예: 1985 驚蟄 진태양시 03-05 23:32)도 같은
오류로 `20日7辰` 이 나왔고 버킷 기준 물리값은 **`21日7辰`** 이다. lunar 流派1 이 23시를 亥(11) 로 두는 특례는 바로 이 문제를 피하는
장치였고, 우리는 23=子 를 택했으므로 날짜를 함께 넘긴다.
규칙: 起運 日 계산용 날짜 = **자시 정책이 정한 명리 날짜**. `JST_2300` 이면 진태양시 23:00 이후는 익일(출생은 `ct.day_offset` 그대로,
절입은 같은 정책 규칙을 `input/time_correction` 의 **공용 함수**로 빼서 재사용 — 복제 금지). `YAJASI_SPLIT` 이면 23시대는 당일(야자시),
00시대만 익일 — 역시 정책 함수 하나가 양쪽을 결정한다. 검증: 앵커 4건을 이 규칙으로 **다시 손계산**하면 ① `24日8辰`=`8年2月20日`→
(8, 1993) ② `21日7辰`=`7年2月10日`→(7, 1992) ③ `21日6辰`=`7年2月0日`→(7, 1992) ④ `21日7辰`→(7, 1992). 이 값들은 lunar 의 옛 값과
우연히 같으므로 **RED 실증은 §5-(나)6·7 과 23시대 전용 앵커로 대신**한다(아래 §5 rev3 항목).
...
9-rev3. **子時 날짜 귀속 양방**: (가) 진태양시 22:30 출생과 23:30 출생(같은 날짜, JST_2300)의 起運 日 이 **1일 차이**가 나야 한다
   (23:30 은 익일 子 → 역행이면 day_diff +1, 순행이면 −1). (나) 절입 진태양시가 23시대인 달(예: 1985 驚蟄 03-05 23:32)에서 절입 날짜가
   익일로 귀속돼 `21日7辰` 이 나옴을 단언. (다) `YAJASI_SPLIT` 정책으로 같은 입력을 넣으면 23:30 이 당일 子 로 남아 日 이 22:30 과
   같음 — 정책 함수 하나가 양쪽을 결정함을 실증. (라) 교정 전 RED: rev1 구현은 (가)에서 1일 차이가 나지 않으므로 RED 가 되어야 한다 —
   rev1 워킹트리에서 먼저 돌려 출력을 첨부.
