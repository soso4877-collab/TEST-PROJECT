...
> [2026-07-03 측정 완료 — 로드맵 원안 정정] 착수 세션에서 fork 를 실측해 설계 확정. 아래 원안의
> "iztro 당일 고정 + engine 비교자 정정"은 **측정 결과 틀린 전제**였다(정정본이 이 블록 뒤 '측정 확정 설계').
- 확정 실측1(setSect): lunar-python `getEightChar().setSect(1)` 은 **일주만 익일 전환하고 시/월/연주는
  보존**한다. 진태양시 1999-06-11 23:18 → sect2(기본): day=甲午·time=丙子 / setSect(1): day=乙未·
  time=丙子(불변)·year=己卯·month=庚午(불변). 즉 JST_2300(23시부터 일주 익일) = setSect(1).
- 확정 실측2(fork — iztro): 같은 케이스에서 자미 chinese_date = `己卯 庚午 乙未 丙子` = **일주 이미
  익일(乙未)**. iztro 는 자시를 익일로 처리한다(로드맵 원안의 '晚子時=당일 고정'은 오류). 현재 명리는
  sect2 버그로 당일(甲午)이라 명리(甲午)↔자미(乙未) **불일치 → 이 자시 케이스는 지금 CALC_MISMATCH 로
  이미 차단 중**. 명리에 setSect(1) 적용하면 명리도 乙未 → 자미와 **자연 일치**.
- day_offset 은 이미 정책을 정확히 반영(time_correction.py:103-109): JST_2300=진태양시 23시+ → 1,
  YAJASI_SPLIT=조자시(00~01)만 1·야자시(23~24)는 0.

**측정 확정 설계 (다음 세션 구현 — 이대로):**
- calc/myeongni.py:145 — `ec = solar.getLunar().getEightChar()` 다음에 `if ct.day_offset: ec.setSect(1)`.
  (day_offset 이 정책 반영값이므로 이 한 줄이 JST/YAJASI 분기를 정확히 수행. 하드코딩 아님.)
- calc/partner.py:120-122 — 동일(`ct = tc.correct(...)` 경로 확인됨, ct.day_offset 접근 가능).
- **engine.py 무수정** — 자미가 이미 익일이라 명리 setSect(1) 후 자연 일치. 로드맵 원안의 day_ganzhi_civil
  비교자 추가는 **하면 안 됨**(당일 甲午 vs 자미 乙未 로 새 불일치 유발). ziwei.py·docs/03 도 무수정.
- getYun(대운)은 line 156 에서 setSect **후** ec 로 도는데, 대운 시작나이/방향이 자시 케이스에서
  바뀌면 setSect 가 일주 밖으로 누수하는 것 → 골든으로 '대운 불변' 반드시 검증(advisor).
- [YAJASI 잔여] YAJASI_SPLIT 23~24시(day_offset=0)는 명리 당일 vs 자미 익일 불일치 가능 — 실운영은
  JST_2300 확정이라 우선순위 낮음. 새 세션에서 YAJASI 자시 케이스 처리(자미도 정책 반영 or 문서화) 결정.
- 골든(같은 커밋): **자시 골든의 기대값은 정책 반영 익일로 first-principles 재도출**(60갑자 甲午+1=乙未,
  JST_2300=23시→익일 per docs/03·절대규칙6). code-match(코드 출력에 맞춤) 절대 금지 = 골든 원죄.
  비-자시 골든 22건은 day_offset=0 → setSect 미발동 → 바이트 동일(영향 0). 신규 앵커:
  (a) JST_2300 진태양시 23:30(6/11)·00:30(6/12) → **같은 일주**(자시 연속성), (b) JST vs YAJASI at
  23:30 → **다른 일주**, (c) 자시 케이스 day/일간/십성/격국/신살 함께 변·월주/연주/대운 시작나이·방향 불변.
- 영향 보고: 수정 후 "결과가 달라지는 입력 구간"(진태양시 23:00~24:00, 시민시각 대략 23:32~00:32)을
  STATE 에 명시. 기존 발송물 재검토는 운영자 판단(보고만).
