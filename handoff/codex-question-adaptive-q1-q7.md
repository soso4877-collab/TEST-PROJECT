...
- 브랜치 `codex/gunghap-relationship-quality`, HEAD `d2f3a4d`(지시문·리뷰 기록 정리 커밋).
...
- `grep -rn "가현\|상철\|태수\|태성\|순조" sajugen/` 0건(테스트 픽스처 포함).
...
- **최초 고객 특정 키워드 일반화**: `_AXES` 안 "김포"·"계양"·"청마"·"장재화", `_PROVENANCE_CONTEXT_TERMS`(310) "청마", `rules.py:756-765`의 동일 키워드 — 실명("장재화")·고유 모임명("청마")은 **제거**하고 일반어(지역 비교, 모임·단체, 도움 주는 사람)로 대체. 지명(김포/계양)은 일반 "지역 비교" 로직으로 흡수. 기존 테스트가 이 키워드에 의존하면 픽스처도 합성 일반어로 치환(PII-인접 제거가 목적이므로 테스트 보존을 이유로 실명을 남기지 않는다).
...
- `grep -rn "장재화\|청마" sajugen/ tests/` 0건.
...
## 범위 밖 — 손대지 않음
`sajugen/calc/`·`sajugen/input/`(골든 불변), 상태머신·발송 차단 로직, 가드 3단 완화, followup 범위 밖 거부 철회, docs/23 베타 매뉴얼(별도 보류), lunar-python 등 스택 버전.
