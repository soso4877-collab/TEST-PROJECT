...
from sajugen.content import client_tone_lint, question_router
...
SYSTEM = (
    ...
    "[질문별 풀이 기준] 질문 축을 먼저 확인하고 초기 관계, 장기 관계의 결혼 이행, 재회 가운데 맞는 기준으로 답한다. "
    "부모나 가족의 반대가 질문에 있으면 가족과의 조율 조건을 별도 판단 축으로 다룬다.\n"
    "[행동] 초기 관계는 연락과 약속, 장기 관계는 생활 조건과 가족 조율, 재회는 이전 문제의 변화와 재접촉 조건을 "
    "각각 구체 기준으로 쓴다. 질문에 없는 새 만남이나 고백을 임의로 전제하지 않는다.\n"
...
GUIDE = {
    ...
    "consult": "질문 분류와 질문 축을 따라 결론, 시기, 바로 확인할 행동을 앞에서 직접 답한다.",
    ...
    "social": "질문별 풀이 기준에 맞춰 가족이나 주변 사람과 관계를 조율하는 방식을 쓴다.",
    "pace": "두 사람이 관계의 다음 단계를 확인하는 속도가 다를 때 어떻게 맞추는지 쓴다.",
    "risk": "관계가 흔들릴 수 있는 장면과 미리 조심할 기준을 쓴다.",
    "stability": "서로에게 좋은 영향이 되는 조건과 안정 관계로 가는 조건을 쓴다.",
    "action": "질문별 풀이 기준에 맞는 다음 행동과 확인 순서를 제안한다.",
    "timing": "앞으로 1년을 중심으로 성급히 밀어붙일 때와 확인할 때를 나누어 말한다.",
    "decision": "관계를 이어갈 기준, 멈출 기준, 다음 단계를 결정할 조건을 제시한다.",
}

_FOCUS = {
    "overview": "먼저 두 사람의 전체 궁합과 현재 질문의 핵심을 답합니다.",
    "consult": "신청한 질문의 결론과 시기, 지금 확인할 행동을 먼저 답합니다.",
    ...
    "social": "가족과 주변 사람 사이에 관계를 놓는 방식도 안정성을 봅니다.",
    ...
    "action": "지금 할 행동은 질문의 단계에 맞춰 확인할 기준을 세우는 쪽으로 봅니다.",
    "timing": "앞으로 1년 안에서 서두를 때와 확인할 때를 나누어 봅니다.",
    "decision": "관계를 이어갈 기준, 확인해야 할 조건, 물러설 기준을 제시합니다.",
}

_AXIS_KEYWORDS = {
    "초기 관계 확인": ("썸", "호감", "고백", "소개팅", "새 만남"),
    "결혼 이행": ("결혼", "혼인", "배우자", "상견례"),
    "부모와 가족의 동의": ("부모", "어머니", "엄마", "아버지", "가족", "반대", "허락"),
    "장기 관계의 다음 단계": ("장기", "오래", "몇 년", "3년", "오랜 연애"),
    "재회 가능성과 조건": ("재회", "헤어진", "전남친", "전여친", "다시 만나"),
    "상대의 진심": ("진심", "마음", "호감", "상대방"),
...
_FRAME_CONTEXT = {
    "early": (
        "초기 관계 풀이 기준: 호감의 크기를 단정하지 말고 연락, 약속, 갈등 뒤 회복이 반복되는지로 "
        "관계를 시작할 조건을 답합니다."
    ),
    "longterm_commitment": (
        "장기·결혼 이행 풀이 기준: 이미 이어 온 시간을 출발점으로 두고 생활 기준, 돈 관리, 역할 분담, "
        "결혼 준비의 다음 단계를 답합니다."
    ),
    "family_commitment": (
        "부모·가족 조율 풀이 기준: 두 사람의 결혼 의지와 함께 반대 이유, 신뢰를 쌓을 시간, 인사와 설득의 "
        "순서를 분리해 답합니다."
    ),
    "reunion": (
        "재회 풀이 기준: 헤어진 원인이 실제로 달라졌는지, 연락을 다시 열 조건과 같은 문제가 반복될 때 "
        "멈출 기준을 답합니다."
    ),
    "general": (
        "일반 관계 풀이 기준: 특정 단계를 임의로 전제하지 않고 관계의 안정성, 대화, 다음 행동을 "
        "질문에 드러난 범위 안에서 답합니다."
    ),
}
...
_RAW_REPLACEMENTS = (
    ...
    ("일지 해", "생활 자리에서 엇갈리기 쉬운 결"),
    ("일지 파", "생활 자리에서 매듭이 흔들릴 수 있는 결"),
    ("일지 원진", "생활 자리에서 이유 없이 서먹해질 수 있는 결"),
    ("일지 자형", "생활 자리에서 비슷한 반응이 스스로를 조일 수 있는 결"),
    ("일지 상형", "생활 자리에서 예의와 속도가 부딪히기 쉬운 결"),
    ("일지", "생활 자리"),
    ("천간합", "서로 묶이는 결"),
    ("오행", "다섯 기운"),
    ("신강약", "힘의 균형"),
...
def build_context(
    ...
    profile = question_profile(situation)
    axes = profile["axes"]
    body = (
        f"{_FOCUS.get(section_id, '')}\n\n"
        f"{profile['instruction']}\n\n"
        f"질문 분류: {profile['category']}.\n\n"
        f"질문 축: {', '.join(axes)}.\n\n"
        f"{unknown_note}\n\n"
...
def question_profile(situation: str | None) -> dict[str, object]:
    """질문을 결정론 풀이 기준으로 바꿔 context·폴백·요약이 함께 소비하게 한다.

    분류 결과를 실제 산출 분기에 연결해 팬텀 라우팅을 막는다. 여러 축이 겹치면 재회,
    부모·가족 조율, 장기·결혼, 초기 관계 순으로 더 구체적인 기준을 우선한다.
    """

    text = (situation or "").strip()
    category = question_router.classify(text)
    axes = _question_axes(text)
    if not text:
        frame = "general"
    elif "재회 가능성과 조건" in axes:
        frame = "reunion"
    elif "부모와 가족의 동의" in axes:
        frame = "family_commitment"
    elif any(axis in axes for axis in ("결혼 이행", "장기 관계의 다음 단계")):
        frame = "longterm_commitment"
    elif "초기 관계 확인" in axes or category == question_router.QuestionCategory.LOVE:
        frame = "early"
    else:
        frame = "general"
    return {
        ...
        "frame": frame,
        "axes": axes,
        "instruction": _FRAME_CONTEXT[frame],
    }
