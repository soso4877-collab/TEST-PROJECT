...
> 최종 갱신: 2026-06-13  (Phase 8 MVP 릴리스 게이트 완료 — delete_order 하드삭제+감사·README-ops·E2E 3건(평일/윤달/시진불명), pytest 153 PASS)
...
Phase6 검수 UI 1차 MVP(2026-06-13, 플랜 sleepy-sleeping-puppy 승인): 운영자 결정 = "완성 우선,
  ...
  다음 후보(Phase 9): 익명/가명 계산특이점 데이터셋 — 삭제 전 식별자(이름·출생지·고민 원문·order_id 연계)
    제거 후 계산사실+경계플래그만 익명 골든셋으로 추출(개인정보보호법 제28조의2 과학적연구·통계 / 제58조의2
    익명정보 적용제외 근거). 그 외: 검수 UI 2차(섹션 재생성·하이라이트·diff), 운영자 확인 2건(표지 표제·한지),
    실주문/지인 베타 2~3건, 성능 ① solar_term_time 캐싱(calc, 골든 회귀 동반).
