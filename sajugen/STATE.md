...
>   ★ 다음 작업 = Phase 3 마지막 **T3.5(a) 목차 재넘침 방어** 하나(B-8·B-5 완료). 템플릿 재설계 = 신선한
>       컨텍스트 강력 권장. **[측정 완료]** 현행 목차 = 14행/페이지(실측, 합성 장17/20/22/24 전부 목차 2페이지:
>       page2=14행 + page3=나머지, first_body_page=4). 즉 로드맵의 '장>18' 뿐 아니라 이미 15장부터 2페이지.
>       22행 1페이지엔 (i) 행높이 ~36% 압축(열화 위험) 또는 (ii) **2단 목차**(22/2=11행/단, 14 용량 여유)
>       필요 → **2단 목차 권장**(행 크기 보존). **[운영자 결정 대기]** 목차 시각 재설계라 2단 vs 압축은
>       운영자 승인 후 진행(붓글씨 폐기·장마다새페이지처럼 운영자 시각 결정 이력). report.html.j2 .toc/.toc-row
>       + pdf.py 에 toc 장수 주입 + 장22개 합성 렌더로 1페이지 증명 + 저밀도/기하 clean 회귀.
>     상세 = 아래 'Phase 3' 블록 + roadmap T3.5. [B-8·B-5 완료는 아래 블록 참조]
...
>   [Phase 3 T3.5 (a) 측정 2026-07-03 — 미구현] 목차 재넘침 방어. 합성 실렌더 실측:
>     현행 목차 14행/페이지, 장 17/20/22/24 전부 목차 2페이지(page2=14행, page3=나머지, first_body_page=4).
>     로드맵 '장>18'보다 낮은 15장부터 이미 2페이지. 22행 1페이지 = (i)행높이 ~36% 압축(열화 위험) 또는
>     (ii)2단 목차(11행/단) → 2단 권장. **운영자 시각 결정 대기**(2단 vs 압축). 구현 시 report.html.j2
>     .toc/.toc-row 재설계 + pdf.py toc 장수 주입 + 장22개 합성 렌더 1페이지 증명 + 저밀도/기하 clean 회귀.
>   ★ Phase 3 남은 태스크 = **T3.5(a) 목차 재넘침 방어** 하나(위 측정·권장 참조, 운영자 결정 후 신선 컨텍스트).
