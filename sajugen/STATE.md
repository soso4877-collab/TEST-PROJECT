...
Phase 0(docs 11종·정책 고정) 완료. 다음 = Phase 1 KASI(운영자 키 발급 필요).
...
>> (진행 중 2026-06-10) 상용화 플랜 Phase 1 = KASI 검증층.
   선행(운영자): data.go.kr 회원가입 후 활용신청 2건(자동승인·무료) —
   음양력 정보 15012679 + 특일 정보 15012690 → 인증키를 .env 에
   KASI_API_KEY 로 저장. 키 확보 후: (1) 1905/1950/2000/2026 실호출로
   get24DivisionsInfo kst(절입시각 분단위) 필드·과거 커버리지 확정,
   (2) scripts/kasi_dump.py 로 1900~2050 전수 캐싱(개발계정 1만건/일,
   약 6일 분할) → data/kasi_cache.sqlite, (3) calc/kasi.py +
   crosscheck 3원 확장 + tests/test_kasi.py.
