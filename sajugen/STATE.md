...
>
>   [★ 내일 "이어받아" 하면 무엇이 시작되는가 — 오해 방지]
>     "이어받아" 는 manifest 를 정본으로 읽으므로 **기본 진입점은 N-1(사주 도메인) 하나**다.
>     아래 [운영자 결정 대기] 의 도구·권한 항목은 manifest 에 없으므로 **자동으로 시작되지 않는다.**
>     그쪽을 하려면 운영자가 그 회차에 **명시적으로 지목**해야 한다("커맨드 승인 부분 하자" 등).
