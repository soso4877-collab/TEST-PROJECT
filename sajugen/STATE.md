...
>     **[2026-08-21 진단 정정 — 근거 `docs/26-engine-eval-2026-08-20.md` §3, docs/16 QI-2026-08-17-01 이월란 병기]**
>     진범은 "달력 앵커"가 아니라 **시진(時辰) 버킷팅**이다. `getSolar` 가 (a)거리끝점=절대축 ·(b)시진버킷=국지축
>     ·(c)달력앵커=시민KST축 **세 역할을 겸직**하는데, CST=KST−1h 라 **홀수시 출생만 시진 버킷을 건너뛴다**
>     (짝수시는 −1h 해도 동일 시진). 층화 실측(격자 25,440건 — 균일무작위 3,000건과 **표본 설계가 달라 합산 금지**):
>     전체 1.42% / 홀수시 2.00% / 짝수시 0.05% / 00시대 2.82% = **홀수시가 짝수시의 40배**.
>     → **달력 앵커(c)만 고치면 약 87% 가 잔존** — (c) 단독 수정은 유효한 수정이 아니다.
>     **심각도 상향**: "전파 한정적" 판정 철회. 불일치 폭에 **±9~±11년 꼬리 8.2%** 가 있고 ±10년이면 대운 슬롯이
>     한 칸 밀려 **`current_daewoon()` 의 "현재 대운" 간지 자체가 달라진다**(2026-06-14 정미/병오 혼서와 동계열).
>     **[2026-08-21 재정정]** 감지구멍은 "골든 ±2 허용폭" 이 아니다 — `:221` 은 자기정합 불변식이라 프레임이 밀리면
>     start_year·start_age 가 함께 밀려 **허용폭과 무관하게 통과**한다. 골든은 `test_named_cases:344` 로 대운수
>     **절대값 7건을 핀**하고 있고, 진짜 구멍은 **그 7건이 전부 (b) 비영향 구간**이라는 커버리지 갭이다
>     (실측: 현행 CST↔진태양시 시진 상이 **0/7**, 축 B 선택 시 **3/7**). 회귀 핀은 허용폭이 아니라
>     **종류가 다른 단언**(프레임 간 대조·절대 start_year)이어야 하고 케이스를 영향 구간에서 골라야 한다.
>     처방 = 起運 계산을 lunar-python 에서
>     회수(거리=Skyfield 초단위 · 이산화=결정된 축 · 앵커=시민KST), **(b) 축 유파 결정이 선행**. ·
