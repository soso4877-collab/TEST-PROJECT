...
풀이재설계(2026-06-12, 커밋 194ec2d): 운영자 "AI가 뽑아낸 것 같다·가독성 최악". 금강산 PDF=하한, 그 이상 목표.
  ...
  미해결(다음): compose 신뢰성 — API 불안정으로 3/12 챕터 가끔 폴백(비결정적). 폴백은 깨끗하나 덜 풍부.
    특히 consult(개인화 피크) 폴백 시 일반 안내로 강등. 신뢰성 하드닝(순차+백오프 or 큐) + 폴백 룰 산문 풍부화 필요.
    PDF veraPDF/게이트 재측정·STATE 갱신 후 운영자 육안 A/B.
  다음(별도): 검수 UI 연결.
