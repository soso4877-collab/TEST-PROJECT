...
> ===== ★ 최신 앵커 (2026-08-23 — **N-5 조사 종결, 운영자 결정 O1 수용+기록, 코드 무변경**) =====
>   [한 줄] factcheck 일상어 동형 예외(11.5%) / "다인 allow-set 공유"는 오진단 → 실체는 토큰 수준 factcheck 의
>   귀속 무지(억제 연·월주가 본인 허용집합에 이미 존재 63.6~74.2%). 2차 벨트는 경계일 억제의 약 2/3 에서 no-op,
>   1차 방어(문안 제외+프롬프트 일주만)가 실제 방어. O2 deny-set(본인 정당 언급 오차단)·O3 귀속 파서(B-8) 기각.
>   [증거] 합성 프로브 852 경계일 전수(1960~2030), PII 0, LLM·PDF 0. 정본 = `docs/16` QI-2026-08-19-01 잔여 (2) 하위 블록.
>   [상태] 활성 패킷 없음. 기준선 **1303 passed / 14 skipped** 불변(코드 diff 0). 사주 도메인 이월 **0건**
>     (N-1·F-1·N-5 전부 종결). 남은 건 상위 프레임 `docs/26` §5 웹앱 이식 검토와 베타 발급 트랙(운영자 지목 시).
>   [다른 세션 담당] 모델 운영 정책 정본 개정(vault `70_AI-Collab/모델-운영-정책.md`).
>
> ===== 압축 재개 앵커 (2026-08-23 — N-5 조사 착수 직전, 위 블록으로 대체됨) =====
...
>
>   [★★ 진입점 — 운영자 지시 2026-08-22. 이 줄이 우선한다]
>     **첫 작업 = `brainhealth-evidence-audit-optin-gap-20260822`** (manifest 고정, `planned / codex`).
>     vault 감사기 `brain-health.ps1` 의 근거 검사가 **`[공식확인]` 리터럴 태그를 단 절만** 평가해
>     (113행) 태그를 안 달면 영구 면제된다. **실측: 정본 88건 중 태그 보유 13 = 75건이 검사 면제.**
>     ★ 가설이 아니라 **실현된 누출 1건 확인** — `70_AI-Collab/컨텍스트-엔지니어링-삭감기준` 이
>     외부 URL 1건·Markdown 인용 0 인데 `grade: 확정`·`evidence: 공식문서` 로 이미 통과했다.
>     URL 기준으로 바꿔도 신규 적발은 **6건뿐**(홍수 아님). 신규 키는 **warning** 으로 내 `exit 0` 유지.
>     · 대상이 **vault(`~/AI-Brain/_scripts`)** 라 이 저장소에 커밋되지 않는다. Codex 접근성 확인이 §0.
>     · 구현자 = Codex(운영자 지정). 오늘 회수한 논문 방향(Codex 구현 → Claude 리뷰 +18.1%p)과 일치.
>
>   [대기열 — 위 태스크 뒤 순서대로]
>     1. `guard-false-positive-triage-20260822` — 가드 오탐 7건 원인 확정(1단계, 완화 금지).
>        패킷 존재, manifest 에서만 내려옴. 재활성하면 된다.
>     2. **N-1** `partner-ym-flag-direct-access-20260822`(사주, 패킷 SHA `b778f115…`).
>     3. 정본 `grade: 검증중` 19건 정리 — 판정표를 `10_Inbox/2026-08-22-정본-검증중-19건-판정표.md` 에
>        만들어 뒀다. ★ **A 그룹 일괄 승격은 하면 안 된다** — 표본 대조에서 4건이 URL 만 있고 원문 인용이
>        0 이라 근거 규율상 `[추론]` 강등 대상이고, 즉시 승격 가능한 건 2건뿐이다(실측).
>
>   [옛 진입점 기록] 직전에는 `guard-false-positive-triage` 가 첫 작업이었다. 근거는 운영자 판단 —
>     **도구 마찰은 모든 후속 작업에 곱해진다.**
>     오탐 7건 중 둘은 사소하지 않았다(하나는 작업 발주 자체를, 하나는 커밋 메시지를 막았다).
>     본 프로젝트를 매끄럽게 하려면 이 층을 먼저 정리한다.
>     · **1단계는 원인 확정까지다. 완화·수정 금지**(교차검토 지시 수용).
>     · 소스 가설(확정 아님): `security-guard.mjs` 68-75행이 Edit/Write 는 **대상 경로만**,
>       shell 은 **command 전체**를 검사하는 비대칭이라 따옴표 안 인자 텍스트가 경로로 오인된다.
>       `DESTRUCTIVE`(89행)도 같은 구조. 7건 각각을 `evaluateRequest` 합성 프로브로 재현해 확정한다.
>     · 대상이 **저장소 밖 전역 훅**(`~/.ai-harness`)이라 2단계 구현 위치는 운영자 결정 사항이다.
>     · **N-1(사주)은 대기열 보존** — 패킷 `handoff/tasks/partner-ym-flag-direct-access-20260822.md`
>       (SHA `b778f115…`)는 그대로 있고, 가드 1단계 뒤 재활성하면 된다(ephemeris 때와 같은 절차).
>
>   [★ "이어받아" 진입점 규칙 — 오해 방지]
>     "이어받아" 는 manifest 를 정본으로 읽는다. 위 진입점 변경이 manifest 에 반영돼 있으므로
>     내일은 가드 태스크로 들어간다. manifest 밖 항목은 **자동으로 시작되지 않으며**
>     운영자가 그 회차에 **명시적으로 지목**해야 한다.
>     세션 규칙·역할 계약은 별도로 기억할 필요가 없다 — `AGENTS.md`(공유 계약·플러그인 함정 5건),
>     `.claude/rules/00-immutable.md`(절대규칙), `.claude/rules/10-methodology.md`(방법론),
>     Claude 자동 메모리(교차리뷰 4파일 경계·handoff 규약)가 세션 시작 때 자동 로드된다.
>
>   [운영자 결정 대기 — manifest 밖. 지목해야 착수된다]
>     · ~~push 여부~~ **완료(2026-08-22)**: feat·main 둘 다 origin push. `main` 을 `8fbc264` 로
>       fast-forward 전진(0c93f98 에서 39커밋 뒤처져 있었고, 그 사이 계산 정확도 fix 3건
>       `fae34f7`·`1151483`·`19a0862` 이 들어가 복구 기준점이 결함 상태였다). 로컬·원격 4개 지점 동일.
>       ★ 단 **main 전진 ≠ 발송 준비 완료.** 톤 개편 이후 실 PDF·실모델·육안 검수 **0건**이고
>       베타 발급은 여전히 보류다. 발송은 APPROVED 상태머신이 계속 물리 차단한다.
>     · **#2-a 자동승인 해제 · `/sandbox` Windows 가용성** — 교차검토가 auto mode 권고 철회를
>       요구했고 수용했으므로 어시스턴트가 먼저 밀지 않는다. 대가는 분류기 미검출률 17%.
>     · **[★ 기록 갭 — 다음 회차 첫 5분에 처리할 것]** 2026-08-22 세션에서 "현역 상위 사용자들이
>       Claude Code + Codex 를 어떻게 쓰는가" 조사를 하고 **변경안 5건**을 도출했는데, 파일에 남은 것은
>       (a) 완료 3건(Codex 플러그인 설치 · PowerShell allowlist 18 · 전역 model 별칭 전환)과
>       (b) 미완 2건(#2-a·#2-c)의 **결론뿐**이다. **조사 근거·출처·나머지 항목의 원문은 미기록**이고
>       압축된 대화에만 있었다. 복구원 = 세션 전사
>       `~/.claude/projects/C--Users-pc-test-project/d015b01a-4c2f-4c67-a5e8-d36ba8e550f4.jsonl`.
>       ★ 재구성해 적을 때 **기억으로 채우지 말 것** — 전사에서 확인된 것만 옮기고 나머지는 "확인 불가".
>     · **#2-c 가드 오탐 정밀화** — 오탐 7건이 근거(전부 실행이 아니라 **텍스트 언급**이었고,
>       하나는 작업 발주를 하나는 커밋 메시지를 막았다). 사건별 차단 코드 확정 전 `deny` 완화 금지.
>       ★ 관련 실측: 권한 평가 순서는 **deny → ask → allow** 이고 `allow` 는 `deny` 를 못 이긴다.
>       **auto mode 를 켜도 `deny` 는 남는다** → 삭제 오탐은 auto mode 로 안 풀린다.
>       (출처 `code.claude.com/docs/en/permissions`.) 파일 삭제는 운영자가 `! rm` 으로 직접 한다.
>
...
>   [남은 항목 — 사주 도메인(이 세션 이전부터 이월. 도구 정비에 묻히지 말 것)]
>     · **F-1 대운 start_year 축** — 위와 동일 건. 이월 3건 중 가장 크다.
>     · **N-1** `content/rules.py:2147` 의 `getattr(pf,"ym_time_dependent",False)` → 직접 접근으로 좁히기.
>       duck-typing 이 계약 위반을 가린다는 지적. **제품 코드 변경, 소규모.** 출처 = partner-unknown-time-boundary
>       교차리뷰 비블로커 소견.
>     · **N-5** factcheck 일상어 동형 예외 / 다인 allow-set 공유. **구조 변경 + 운영자 판단** 필요.
>       (A 의 억제 간지가 B 의 실제 간지와 겹치면 허용되는 문제.)
>     · 상위 프레임 — `docs/26` §5 웹앱 이식 경로(A 배치 사전계산 + B 내부 API 혼합)와 §6 마스터플랜 정정 6행.
>       ephemeris 절대경로 제거가 그 **선결 조건**이다.
>   [push] 0 — 운영자 지시 대기.
...
>     불변 사본 생성됨. **push 0** — 운영자 지시 대기(`origin/main` 대비 21커밋 앞섬).
>     base `c280a98` 위 커밋 4건 — 제품+테스트+정책문서 `19a0862` / 인계 기록 `3d35c21` /
>     커밋 SHA 기록 + N-2 교정 `c7e9c78` / 마감(archive) = 이 앵커를 담은 커밋 자신
>     (자기 SHA는 기재 불가 — 해시 스냅샷 순서상 write 가 커밋보다 앞선다. `git log --oneline -5` 로 확인).
>     커밋 세션 재확인(HEAD `c7e9c78` 트리): 전체 **1266 passed / 4 skipped / exit 0**(200.51s), `git status --porcelain` 무출력,
>     `handoff.mjs validate` exit 0. 제품 diff 는 리뷰가 본 것과 동일(`c280a98..HEAD` = partner.py 24 / builder.py 11 / rules.py 16 줄).
>   [다음 세션] **활성 패킷 없음.** 새 패킷을 발주해야 착수 가능하다 — 후보는 아래 [이월] 참조.
...
>   [이월 — 다음 패킷 후보] **N-1** `rules.py:2147` getattr → 직접 접근(제품 코드 변경, 소규모) ·
>     **F-1** 대운 start_year 축(유파 판단 + `docs/03` 변경 필요 — 패킷 설계부터).
>     **[2026-08-21 진단 정정 — 정본은 `docs/16` QI-2026-08-17-01 이월란 F-1 의 2026-08-21 블록.
>     ★커밋 `27464c3`·`488093b` 의 메시지 본문은 폐기된 주장을 담고 있으니 git log 를 근거로 쓰지 말 것]**
>     하루에 세 번 손댔다: 원인 전복(c2) → 감지구멍 재정정(c3) → **별도 세션 교차검토가 c2·c3 양쪽을 반증**,
>     그 반증을 전부 재현·확인한 결과가 아래다.
>     · **원인**: `getSolar` 가 (a)거리끝점 ·(b)시진버킷(`Yun.py:39-52`) ·(c)달력앵커 **세 역할 겸직**.
>       지배적 메커니즘은 **(b)**, (c) 는 00시대 전용. **단 (b) 의 올바른 축이 미결정이라 "오류" 로 확정 안 함.**
>     · **층화 실측(동일 격자 재현)**: 홀 321/12,720=2.52% · 00시 32/1,060=3.02% · 짝(0제외) 8/11,660=0.069%
>       · 전체 361/25,440=1.42%. 00시대 전부 제거 가정해도 **329/361=91.1% 잔존** → **(c) 단독 수정 불충분**
>       (정성 결론만 유지). ~~"약 87%"·"1.1%p 정합"~~ **폐기**(다른 표본 설계 비율의 합성, 재현 불가).
>     · **폭 분포**(A-2 = 연속 격자 **25,056건**, A-1 25,440 과 다른 표본): 불일치 371 = ±1년 337(90.8%)
>       + 꼬리 **32건**(8.6%) + 0년 2. `docs/26` §3-5 의 "30건·91.8%" 는 오기.
>     · **심각도 상향 철회**. `current_daewoon()` 간지 분기 전수 측정 = ref 2026 **36건(전체 0.14%)**,
>       ref 2010·2020·2030 각 34건. **꼬리 기여는 ref2026 에서 2건뿐, 다른 ref 는 0** — 나머지는 평범한 ±1년.
>       꼬리는 간지열이 한 슬롯 정렬돼 대체로 상쇄된다. 혼서 사고 동계열 단정도 철회.
>     · **감지구멍**: `:221` 은 축 간 대조가 없는 구조 불변식이라 대부분 놓친다(유효). 단 ~~"항상 함께 밀려
>       무조건 통과"~~ 는 **거짓** — `1985-03-05 23:30 남` Δqiyun+9/Δsy+10, `1988-01-01 00:30 남` 은 CST 에서
>       **실제 `:221` 실패**. ~~"골든은 눈이 멀지 않았다"~~ 도 **철회** — `_NAMED` 대운수 7개는 `d3b5972`
>       ("손대조 0")의 **구현 스냅샷**이고 `docs/12` §6 독립 오라클에 대운수 절대값 없음.
>     · **축 민감도 `0/7·3/7` 철회** — 그건 **시지** 차이지 대운수가 아니다. `_NAMED` 7건의 (qiyun, start_year)는
>       세 축 전부 동일 **0/7·0/7**. `Yun` 은 시진 외에 `day_diff`(달력 일수)도 쓰고, `hour==23→亥` 특례라
>       `LunarUtil`(子)과도 경계가 다르다.
>     · **축 근거 없음 → `docs/03` 신규 행 등재 보류**. "고전 시진=진태양시" 1차 자료 0건(`docs/00` 검색).
>       다음 패킷 = **유파 1차 자료 조사 + 축별 반사실 프로브**(한 축만 바꿔 기여도 분리). ·
>     **N-5** factcheck 일상어 동형 예외 / 다인 allow-set 공유(구조 변경, 운영자 판단).
...
>     **[2026-08-21 정정]** 이 줄의 "앵커"·"0.53~0.83%" 는 당시 진단 기준이다. 진범은 **시진 버킷팅**이고 심각도는
>     상향됐다 — 최상단 앵커의 정정 블록과 `docs/26` §3 을 정본으로 본다.
