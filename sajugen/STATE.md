...
>   ★ 다음 = **Phase 3 T3.1 A-2**(저밀도 장꼬리 글자수 하한) — verify.py:192 장꼬리 면제(다음이 새 장이면
>     짧은 페이지 제외)에 글자수 하한 도입: 다음이 새 장이어도 len<하한이면 저밀도 hit 유지. 캘리브레이션:
>     DOC_A(sajugen/render/out/customer2_integrated_full.content.json) 재렌더로 장꼬리 글자수 분포 실측 →
>     p16=50자(사실상 빈 페이지)는 잡히고 정상 조판 꼬리(예 90~119)는 면제되는 하한 선택, _LOW_DENSITY_MIN
>     (120)·_ORPHAN_MIN(40)과 관계 명시. 3중 사각(41~하한) 소멸 표 증명 테스트(양방). A-4 는 매칭 확대만이라
>     A-2 와 함께 설계해야(매칭 확대만 하면 사각도 10장 이후로 확대).
>   이어서 T3.2[B-2] 기하게이트+실렌더회귀·T3.3[B-1+G-5] 최종발급 게이트완전화·T3.4[D-1] tagged 항진해소·
>     T3.5[B-4+B-5] 목차재넘침. 전 태스크 정상/결함 양방 회귀 필수(완화 0).
