...
풀이재설계(2026-06-12, 커밋 194ec2d): 운영자 "AI가 뽑아낸 것 같다·가독성 최악". 금강산 PDF=하한, 그 이상 목표.
  ...
  >>> 다음 세션 1순위(운영자 지정 2026-06-12) = compose 신뢰성부터. 현상: API 불안정으로 12챕터 중 3개가
      비결정적 폴백(런마다 실패 챕터 다름 = 속도제한/일시 오류). max_retries=8·동시성3·1회 재시도로도 잔존.
      특히 consult(개인화 피크) 폴백 시 일반 안내로 강등 = 가장 아픔. 폴백 자체는 _clean_display로 깨끗(기호0).
      접근안(택1·실측): (a) 순차 생성 + 지수 백오프(가장 단순·안전, 12챕터 ~12분 감수), (b) 동시성2로 더 낮춰
      TPM 회피, (c) 실패 챕터만 모아 2차 재시도 큐, (d) 실패 원인 로깅 먼저(429/529/timeout/guard 구분 — 현재
      builder _compose_one 의 except 가 원인을 삼킴 → 우선 stderr 로깅 추가해 진짜 원인부터 확인). 목표 12/12.
  이후: 폴백 룰 산문 풍부화 → PDF veraPDF/게이트 재측정 → 운영자 PDF 육안 A/B → 검수 UI 연결.
  착수 전 필독 메모리: feedback-sajugen-llm-content-pitfalls(반복실수 체크리스트).
