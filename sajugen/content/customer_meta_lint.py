...
_RULES: list[tuple[str, re.Pattern[str]]] = [
    ...
    (
        # 작성자 작업 예고 = 문서 내레이터 말투(AI 인상). 실사고 2026-07-05(운영자 발견):
        # v6 1장 골격이 "사주를 한 장으로 압축해 보겠습니다"를 그대로 노출 — 이 규범은
        # llm_sections 프롬프트에만 있었고(LLM 지시) 룰 골격과 lint 는 미동기화였다.
        # '~보겠습니다'는 리포트 문어체에서 작성자 1인칭 작업 예고 전용 표현이라 전면 차단
        # ('~으로 봅니다'(관법 서술)·'보세요'(행동 권유)는 통과 — 양방 테스트로 고정).
        "writer_task_announcement",
        re.compile(r"보겠습니다"),
    ),
    (
        # 빈 지시(일반론): 무엇을 어떻게 볼지 없이 "차례대로/순서대로 확인" 류 지시만 남는
        # 상투구 — 같은 실사고("조건, 사람, 시기를 차례대로 확인하세요")에서 등재.
        "formulaic_empty_instruction",
        re.compile(r"차례대로\s*(?:확인|점검)|순서대로\s*확인"),
    ),
]
