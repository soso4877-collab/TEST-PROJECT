...
_STYLE_REPLACEMENTS = (
    # '또렷이'는 부사 특례 — 어간 캐치올('또렷'→'분명')로 가면 '분명이'가 되므로 먼저 치환.
    ("또렷이", "분명하게"),
    # '또렷' 어간 캐치올(2026-07-05 백로그): '또렷해지는/또렷해집니다' 등 활용형이 개별 표에
    # 없어 style_lint(또렷[가-힣]*)가 정당 폴백 → 윤문 유실(실측: v6 시기재물 폴백 2 중 1).
    # '또렷하다'와 '분명하다'는 같은 '-하다' 형용사라 어간 치환으로 전 활용형이 문법 보존된다.
    # 개별 활용형('또렷하게/또렷한/또렷합니다') 항목은 이 캐치올과 결과가 동일해 흡수했다.
    ("또렷", "분명"),
    ("결을 따라 걷고", "흐름을 차분히 살피고"),
    ("결을 따라 걷", "흐름을 차분히 살피"),
    # P3(2026-07-05): 작성자 작업 예고 선치환 — '-어 보겠습니다' 전 활용에서 문법 보존
    # (살펴보겠습니다→살펴봅니다). customer_meta_lint writer_task_announcement 게이트는
    # 불변(완화 0) — 가드 전 결정론 선반영으로 폴백률만 낮춘다('또렷'→'분명' 선례).
    ("보겠습니다", "봅니다"),
)
