...
def build_all(
    ...
    T["summary"] = (
        ...
        f"[읽는 방향] 큰 그림 → 영역별 흐름 → 시간의 리듬 → 현실 적용 순으로 "
        f"이어집니다. 타고난 결의 흐름이니, {nm_call}의 선택과 쓰임에 따라 "
        f"더 좋게 살릴 수 있습니다."
    )
    ...
    T["strength"] = (
        ...
        f"방향이 보입니다.\n"
        f"겉과 속의 짜임도 함께 보겠습니다. 천간(겉으로 드러나는 성향)은 연 "
    ...
    if is_minor:
        ...
        T["love"] = (
            ...
            f"단정하지 않으며, 관계는 두 사람의 상호작용으로 달라지는 영역으로 봅니다."
        )
    ...
    if _cc == "연애":
        ...
        T["consult"] = (
            ...
            f"두 관점을 함께 읽으면 감정적인 위로에 머물지 않고, 지금 할 일과 조심할 일을 나누어 볼 수 "
            f"있습니다. 이 결과지는 그 기준을 잡기 위한 글로 읽어 주세요."
        )
...
def partner_block(pf, saju, label: str = "", lunar_input: bool = False) -> str:
    ...
    lines = [
        ...
        f"신청자 일간 {my_dm_ko}({my_dm_elem}) 기준으로 {who}의 일간 "
    ...
    if pf.ilji_relation == "육합":
        lines.append(f"신청자 일지와 {who}의 일지는 육합, 서로 끌어당겨 묶이는 자리다.")
    elif pf.ilji_relation == "충":
        lines.append(f"신청자 일지와 {who}의 일지는 충, 강하게 부딪히며 흔드는 자리다.")
    if pf.ilji_banhap:
        lines.append(
            f"신청자 일지와 {who}의 일지는 삼합의 짝, 만나면 {pf.ilji_banhap}의 기운이 "
            f"살아나는 짜임이다."
        )
    if pf.complements_elems_ko:
        lines.append(
            f"{who}의 명식에는 신청자에게 옅은 "
            + ", ".join(pf.complements_elems_ko)
            + " 기운이 들어 있다."
        )
    if pf.matches_my_yongshin:
        lines.append(f"{who}의 일간 오행은 신청자의 억부용신과 같다.")
    return "\n".join(lines)
