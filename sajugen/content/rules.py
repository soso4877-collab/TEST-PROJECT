...
def build_all(
    ...
    T["job"] = (
        ...
        f"월간 십성 {_J(f'{mon_sg}({mon_sgm})', '은는')} {_J(nm_call, '이가')} 일하는 방식의 결을, "
    ...
    T["wealth"] = (
        ...
        f"두고 버는 흐름과 쓰는 흐름의 균형을 한 번 점검해 보시고, {gk}의 결을 "
    ...
    if _cc == "연애":
        T["consult"] = (
            ...
            f"시기와 판단 지점을 먼저 보아야 합니다. 앞으로 1년 안의 질문이라면 시간의 흐름 장에서 "
            f"확인한 가까운 세운을 기준으로, 연락을 다시 꺼내 볼 때와 관계를 정리해 보아야 할 때를 "
            f"나누어 읽는 것이 좋습니다. 좋은 말만 붙잡기보다 상대가 실제로 대화를 이어 오는지, "
            f"약속을 구체적으로 잡는지, 같은 문제가 반복되는지를 같이 보셔야 합니다. 재회나 결혼을 "
            f"단정하지는 않지만, 관계가 움직이는 자리와 무리하면 흔들리는 자리를 먼저 확인하는 것이 "
            f"이 고민의 핵심입니다."
        )
    else:
        T["consult"] = (
