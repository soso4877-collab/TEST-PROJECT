...
def build_all(
    ...
    T["summary"] = (
        ...
        f"나타내는 중심입니다.\n"
        f"[기운 분포] {mx_ko} 기운({mx_mn})이 비교적 강하게 보이고, {mn_ko} "
    ...
    T["howto"] = (
        "이 결과지는 '큰 그림 → 영역별 흐름 → 시간의 리듬 → 현실 적용' 순서로 "
    ...
    if m.shinsal_detail:
        ...
        T["shinsal"] = (
            f"신살(神殺)은 전통적으로 사주의 특정 기운을 표 기준으로 살피는 "
            f"보조 참고입니다. {nm_call}의 사주에서는 길신·살을 기준으로 두드러지게 "
    ...
    if is_minor:
        ...
        T["love"] = (
            ...
            + " 관계의 리듬이 한결 편해집니다. 특정한 만남이나 헤어짐을 "
    ...
    T["wealth"] = (
        ...
        f"세운 흐름이 바뀌는 전환기에는 관리의 리듬도 한 번 조정해 보세요. "
    ...
    T["health"] = (
        f"이 장은 병을 진단하는 자리가 아니라, 몸과 마음의 리듬을 살피는 "
    ...
    T["monthly"] = (
        ...
        + (
            ...
            f"추진과 점검, 뒤쪽은 마무리와 정리의 순서로 리듬을 잡는 참고가 "
            f"됩니다.\n"
            if wol_q
            else "한 해를 분기로 나눠 시작·추진·마무리의 결로 리듬을 잡는 식으로 봅니다.\n"
        )
        + f"달마다의 흐름은 일과 관계의 리듬을 잡는 참고로 쓰시고, 구체적인 "
    ...
    if seun_fwd:
        ...
        T["seun"] = (
            f"세운(歲運)은 해마다 바뀌는 흐름, 월운(月運)은 달마다의 리듬입니다. "
        ...
        T["seun"] = (
            "세운(해마다의 흐름)과 월운(달마다의 리듬)은 대운의 큰 흐름 위에서 "
    ...
    if _cc == "연애":
        ...
        T["consult"] = (
            ...
            f"{_near_label}는 감정의 크기를 키우는 구간이 아니라 확인의 횟수를 늘리는 구간으로 "
            f"쓰십시오. 관계의 속도는 확인이 끝난 만큼만 올리는 것이 이 판에 맞는 방식입니다."
        )
    elif _ctx_topics:
        ...
        T["consult"] = (
            ...
            + _cp_line
            + f"{_near_label}는 조건을 확정하고 가장 작은 것부터 실행해 반응을 확인하는 구간으로 "
        ...
        T["consult"] = (
            ...
            f"시기는 {_near_label}가 판을 정리하기에 가장 유리한 구간입니다. 이 구간에는 "
            f"새 판을 벌리는 일보다 걸려 있는 일의 조건을 확정하는 일이 먼저입니다. "
            f"확정된 것과 말뿐인 것을 나누고, 말뿐인 것은 기한을 정해 확인하십시오. "
            f"구간 안에서도 무게 배분이 있어, 앞쪽은 확인과 정리에, 뒤쪽은 실행과 "
            f"결정에 힘을 실으면 같은 힘을 두 번 쓰지 않아도 됩니다. 그 다음 해부터는 "
            f"정리된 판 위에서 넓히는 흐름으로 이어 가면 됩니다.\n\n"
            f"조심할 자리도 분명합니다. {mn_ko} 기운({mn_mn})이 옅은 구성이라, "
            f"{mn_mn}이 필요한 국면, 그러니까 서류를 맞추고 숫자를 확인하고 끝맺음을 "
            f"점검하는 자리에서 서두르면 빈틈이 생기기 쉽습니다. 빈틈은 대개 큰일이 "
            f"아니라 사소한 확인을 건너뛴 데서 생깁니다. 이 대목만 의식적으로 "
            f"한 박자 늦추면, 강한 {mx_ko} 기운이 만들어 둔 판을 안전하게 지킬 수 "
            f"있습니다.\n\n"
            f"오늘 시작할 첫걸음은 종이 한 장이면 충분합니다. 이 고민에서 {nm_call}이 "
            f"직접 결정할 수 있는 것과 상대나 상황에 달린 것을 나누어 적고, 직접 결정할 "
            f"수 있는 것 가운데 가장 작은 하나를 {_near_label} 초입에 실행해 반응을 "
            f"보십시오. 상황에 달린 것은 기다리되 기한을 정해 두십시오. 그렇게 하면 이 "
            f"질문의 답이 감이 아니라 확인된 사실 위에 서게 되고, 그때부터는 같은 고민이 "
            f"같은 무게로 돌아오지 않습니다."
        )
