...
def _build_three_pillar_all(
    ...
    *,
    ...
    identity_line = (
        f"일간 {_J(dm_ko, '은는')} 자기 중심을 읽는 기준 글자이고, "
    ...
    intro = (
        f"{nm_pfx}먼저 확인할 수 있는 사실과 확인할 수 없는 사실을 나누었습니다. "
        f"출생 시간이 확인되지 않아, 확인된 세 기둥에서 분명히 읽을 수 있는 "
    ...
    nature = (
        ...
        f"연주 {_J(year_gz, '과와')} 월주 {_J(month_gz, '이가')} 보여 주는 바깥 환경의 결, "
        f"일주 {_J(day_gz, '이가')} "
    ...
    consult = (
        ...
        "이 범위를 벗어나는 현실 조건은 명리의 근거로 대신 결정하지 않습니다."
    )
    closing = (
...
def build_all(
    ...
    day_sgm = _ss_mean(m.day.shishen_gan)
    ...
    if is_minor:
        ...
        T["love"] = (
            ...
            "단정하지 않으며, 관계는 서로의 상호작용으로 달라지는 영역으로 봅니다."
        )
    ...
    T["health"] = (
        ...
        + "생활에서는 강한 기운으로 치우쳐 무리하지 않게 휴식의 박자를 두고, "
        "옅은 기운은 수면·식사·운동 같은 생활 습관으로 채워 가는 것이 "
        "좋습니다. 몸 상태 변화는 그때그때 기록해 두고, 회복이 더딘 날은 "
        "무리한 일정을 덜어 내세요."
    )
    ...
    T["thisyear"] = (
        ...
        + "시기 풀이는 '무엇이 정해졌는가'가 아니라 '무엇을 준비하면 좋은가'의 "
        "관점으로 쓰시길 권합니다. 좋게 보이는 시기에는 강점을 펼치고, "
        "조심스러운 시기에는 속도를 고르며, 흐름에 맞춰 완급을 조절하는 "
        "완급을 정하는 단서로 보시면 좋습니다."
    )
    ...
    T["monthly"] = (
        ...
        + "달마다의 흐름은 일과 관계의 속도를 잡는 단서로 쓰시고, 구체적인 "
        "달별 조언은 기준 시점을 정해 이어서 짚어드립니다."
    )
    ...
    if not unknown_time and love_p is not None and job_p is not None and wealth_p is not None:
        ...
        _cross_body = (
            "명리의 대운·세운이 큰 방향을 정하고, 영역별 결은 상황에 따라 나누어 "
            "살핍니다. 두 체계가 같은 출생 정보에서 어긋나지 않는다는 것은 풀이의 "
            "토대가 안정적이라는 근거가 됩니다."
        )
    T["cross"] = (
    ...
    if _cc == "연애":
        ...
        T["consult"] = (
            ...
            + _cp_line
            + "좋은 말만 듣고 버티는 것이 아니라, 언제 다가가고 언제 멈출지를 나누어 보는 것이 "
            "이 판이 가리키는 방식입니다. "
            + _love_close
