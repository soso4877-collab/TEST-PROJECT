...
_COMPOSE_SYSTEM = (
    ...
    "· 결과 보장 금지: '반드시/무조건/틀림없이/확실히/꼭'은 문맥과 상관없이 본문에 쓰지 마라. "
    "'굳이', '차분히', '짚고 가면 좋다'처럼 약한 말로 바꿔라. "
    "'반드시 결혼하게 됩니다', '무조건 막을 필요는 없어요' 모두 금지다.\n"
...
_COMPOSE_GUIDE = {
    "intro": (
        ...
        "처음부터 그 사람 이야기로. 근거 자료에 '신청 질문부터 먼저 답하면'으로 시작하는 문단이 있으면 "
        "그 직답(방향·시기·행동)을 인사 바로 다음 문단에 반드시 유지하라 — 유료 풀이는 첫 화면에서 "
        "질문에 답해야 한다. 그 문단을 빼거나 뒤로 미루면 반려된다."
    ),
    "wonguk": (
    ...
    "love": (
        ...
        "무리하면 흔들리는 지점을 나누어 말한다. 만남·이별·결혼의 결과 보장만 금지."
    ),
    "work": (
