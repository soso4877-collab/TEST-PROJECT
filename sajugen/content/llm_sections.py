...
_COMPOSE_SYSTEM = (
    ...
    "짚되 확정·보장으로 말하지 않는다.\n"
    "· 시기·나이는 추정형으로 흐리지 마라. 세운은 연도가 정해져 있으니 '2026년에는'처럼 연도로 또렷이 "
    ...
    "· 같은 단어를 버릇처럼 반복하지 마라. 특히 '기운·결·흐름·구조'를 문단마다 쓰지 말고 "
    "일상어(힘, 성향, 분위기, 방식, 시기)로 바꿔 가며 써라.\n"
...
_COMPOSE_GUIDE = {
    "intro": (
        ...
        "일간이 어떤 그릇인지 또렷하게 짚는다. 강한 오행과 옅은 오행, 자미두수 명궁의 분위기를 "
    ...
    "nature": (
        "타고난 바탕과 기질을 직설로 풀어 준다. '○○님은 ~한 분이에요'처럼 단정으로. 일간·십성 구조·신강약이 "
    ...
    "love": (
        ...
        "단정으로 짚는다('○○님 연애는 단순하지 않습니다'처럼). 어떤 사람에게 끌리고 어떤 일이 반복되는지, "
    ...
    "closing": (
        "결과지를 마무리하는 따뜻한 격려를 전한다. 그 사람의 강점을 한 번 더 또렷하게 짚고, 약한 부분은 "
