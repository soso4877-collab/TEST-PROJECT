...
_COMPOSE_SYSTEM = (
    ...
    "· 서양 점성술은 이 상품의 근거가 아니다. 별자리·황도 12궁·점성술 해석을 "
    "만들거나 언급하지 마라. 제공된 명식 근거만 사용한다.\n\n"
    "[표기 — 어기면 즉시 폐기]\n"
...
_LAYER_WEAVE = (
    ...
    "읽어라. 앞서 명리로 짚은 구체적 결론(특정 십성·용신·힘의 강약 같은)과 자미두수의 특정 "
    "궁·별이 같은 방향을 가리키면, 그 둘을 나란히 대어 '두 판이 같은 자리를 짚는다'고 사실로 "
    "확인해 주고, 다르게 보이면 명리의 큰 흐름이 진행되는 과정의 국면으로 읽어라(모순이라 "
    "말하지 않는다). '명리로 보면 ~, 자미두수로 보면 ~' 식으로 체계 이름을 앞세워 문단을 "
    "가르는 것도 금지 — 근거는 자연스럽게 문장 안에 녹인다."
)
...
_COMPOSE_GUIDE = {
    ...
    "nature": (
        ...
        "'○○님은 ◇◇일주예요'를 다시 쓰지 말고, 일간의 성질부터 바로 들어간다.) 한 단계씩 인과로 "
        "이어라 — 일간이 어떤 기질이라(일간의 물상 성격은 법칙처럼 단정하지 말고 '이 사람은 ~한 "
        "경향'으로), 힘의 강약(신강·신약)이 그 기질을 어느 방향으로 키우고, 십성 구조가 그것을 일과 "
        "관계의 어떤 방식으로 드러내며, 그래서 삶에서 어떤 패턴으로 반복되는지까지 잇는다. 없는(옅은) "
        "오행은 중립 여백이 아니라 성정의 빈자리로 짚되, 결핍만이 아니라 그 자리를 채우려는 갈망으로도 "
        "함께 읽는다(양가). 사람 자체는 '○○님은 ~한 분이에요'처럼 직설로 그리되, 물상 성격 이론을 "
        "고정된 법칙으로 단정하지는 마라. 좋고 나쁨이 아니라 쓰임의 방향으로, 얼버무리지 말고."
    ),
    "frame": (
    ...
    "closing": (
        ...
        "식의 힘 있는 문장으로 맺는다. 서양 별자리·황도·점성술은 끌어오지 않는다."
    ),
}
