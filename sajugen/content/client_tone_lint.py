...
LOANWORDS: dict[str, list[str]] = {
    ...
    "솔루션": ["해결책", "방안"],
    "루틴": ["습관", "반복 흐름"],
    # P5(2026-07-05, 운영자 지적): 연성 외래어 — 골격 전 소스 스윕(P5-1) 후 등재.
    "리듬": ["호흡", "박자", "흐름"],
    "스케줄": ["일정"],
    "플로우": ["흐름"],
    "세팅": ["설정", "자리 잡음"],
...
_RAW_CALC_HEAD = [
    ...
    re.compile(r"십성축"),
    re.compile(r"신강약"),
    # P5(2026-07-05, 운영자 지적 v7 1장): '대운수 N'·'오행국' 표제어·서술어에 녹은 bare
    # '분포'가 어떤 패턴에도 없던 사각(위 '오행 분포'는 '오행' 인접 전제). 골격 스윕(P5-1)
    # 선행 후 등재 — 완화 0, 사각 축소. bare 분포는 앞이 한글이 아닌 경우만(합성어 오탐 방지).
    re.compile(r"대운수"),
    re.compile(r"오행국"),
    re.compile(r"(?<![가-힣])분포"),
]
...
_TERMS = [
    ...
    "괴강",
    "백호",
    # P5-4(2026-07-05, 조사 A): 십성·사화·궁명·주요 신살이 목록에 없어 밀도 관측 자체가
    # 안 되던 갭 보강 — 여전히 보고 전용(첫 1회 괄호 풀이 정책과의 오탐 충돌로 게이트 비권고).
    "십성",
    "사화",
    "화록",
    "화권",
