...
- 판정: **EVIDENCE_SPLIT_PASS**. rev1 구현을 보존한 채 子時 날짜 귀속을 교정했고, 손계산 앵커·
  23시대 양방·1994 포함 10개년 전수·전체 pytest·지정 묶음·Ruff를 모두 통과했다. 기준환경과
  Codex 샌드박스의 Playwright pass/skip 차이만 분리했다.
- 인계: `node C:/Users/pc/.ai-harness/handoff.mjs validate --repo C:/Users/pc/test-project`
  → `HANDOFF_VALID task_id=daewoon-qiyun-axis-20260823 status=planned next_actor=codex`, exit 0.
  packet SHA-256 `3b07bf6089b09487ae3f14280776db9b387949ee815729b93332a527815e9dea`가
  manifest와 일치했다. HEAD=`2433805ad863aa573e75e1d02b2829b981a0f3e2`.

## rev3 구현

- `sajugen/input/time_correction.py`: `zasi_day_offset()`를 단일 자시 날짜 정책 함수로 추가했다.
  `correct()`의 출생 `day_offset`과 起運 절입 시각이 같은 함수에서 JST_2300/YAJASI_SPLIT을 결정한다.
- `sajugen/calc/myeongni.py`: `compute_qiyun()`의 日 계산 날짜를 출생 `ct.day_offset`과 절입의
  `zasi_day_offset()`이 적용된 명리 날짜로 교체했다. 절대 UTC 거리·국지 時辰·시민 앵커의 나머지
  rev1 설계는 유지했다.
- `tests/test_daewoon_qiyun_axis.py`: 손계산 4건을 rev3 값으로 재계산하고, 출생 22:30/23:30의
  JST↔YAJASI 날짜 귀속 및 1985 驚蟄 진태양시 23:32 절입 귀속 양방을 추가했다.
- rev1의 `advanced.py` 현재 대운/세운 배선과 `test_golden_sweep.py`의 좁은 `{0,1}` 축 단언은
  그대로 보존했다. 가드·기존 단언 완화는 0이다.

## 교정 전 RED와 손계산 앵커

제품 수정 전 rev3 테스트만 바꾼 실행:
Windows `cmd.exe`에서 `.venv\Scripts\python.exe -m pytest <손계산 2함수 + rev3 23시대 2함수> -q`
→ **11 failed in 4.03s / exit 1**. 신규 심볼 부재가 아니라 rev1 실제 결과가 틀린 순수 거동 RED였다.

| 합성 출생·방향 | rev3 三命通會 손계산 | rev1 실제 | rev3 결과 |
|---|---|---|---|
| 1985-01-01 00:30, 역행 | `24日8辰` → `8年2月20日` | `7年10月20日`, 1992-11-21 | `8年2月20日`, 1993-03-21 (`8/1993`) |
| 1985-02-12 11:30, 순행 | `21日7辰` → `7年2月10日` | `6年10月10日`, 1991-12-22 | `7年2月10日`, 1992-04-22 (`7/1992`) |
| 1985-02-12 13:30, 순행 | `21日6辰` → `7年2月0日` | `6年10月0日`, 1991-12-12 | `7年2月0日`, 1992-04-12 (`7/1992`) |
| 1985-02-12 10:30, 순행 | `21日7辰` → `7年2月10日` | `6年10月10日`, 1991-12-22 | `7年2月10日`, 1992-04-22 (`7/1992`) |

교정 후 같은 선택 집합 → **11 passed in 3.84s / exit 0**.

## 1985~1994 서울 격자 전수

표본은 7일 간격 × 24시 × 남녀 × 분=30, 연도별 2,544건, 합계 25,440건이다.

| 연도 | 측정 | 대운수 변동 | `start_year` 변동 | `start_year` 비율 | 현재 대운 선택 변동 | 세운 연도 집합 변동 | 방향/대운간지/세운간지/월운간지 변동 |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 1985 | 2,544 | 20 | 22 | 0.86% | 2 | 11 | 0 / 0 / 0 / 0 |
| 1986 | 2,544 | 21 | 20 | 0.79% | 3 | 10 | 0 / 0 / 0 / 0 |
| 1987 | 2,544 | 33 | 28 | 1.10% | 2 | 11 | 0 / 0 / 0 / 0 |
| 1988 | 2,544 | 25 | 24 | 0.94% | 1 | 10 | 0 / 0 / 0 / 0 |
| 1989 | 2,544 | 26 | 24 | 0.94% | 4 | 18 | 0 / 0 / 0 / 0 |
| 1990 | 2,544 | 24 | 27 | 1.06% | 7 | 18 | 0 / 0 / 0 / 0 |
| 1991 | 2,544 | 24 | 23 | 0.90% | 1 | 10 | 0 / 0 / 0 / 0 |
| 1992 | 2,544 | 28 | 25 | 0.98% | 0 | 15 | 0 / 0 / 0 / 0 |
| 1993 | 2,544 | 25 | 23 | 0.90% | 4 | 8 | 0 / 0 / 0 / 0 |
| 1994 | 2,544 | 14 | 18 | 0.71% | 2 | 6 | 0 / 0 / 0 / 0 |
| **합계** | **25,440** | **240** | **234** | **0.92%** | **26** | **117** | **0 / 0 / 0 / 0** |

- `start_year` 변동률 = **234 / 25,440 = 0.92%** → 패킷 정지선 5% 미발동.
- 단독 격자 명령 → **10 passed / exit 0 / 1188.11s(19분 48초)**.
- 상시 전체 테스트에 넣기에는 약 20분을 단독 점유하므로 **옵트인 분리를 권고**한다. 이번 태스크에서는
  테스트 구조를 바꾸지 않고 지정된 전체 실행에 그대로 포함했다.

## 최종 검증

- 전체: `.venv\Scripts\python.exe -m pytest tests/ -q`
  → **1275 passed / 32 skipped / exit 0 / 1314.07s(21분 54초)**.
  기준환경 기준선 1280+4=1284에 신규 23건을 더한 총수 **1307**과 현재 1275+32=1307이 일치한다.
  이 환경의 추가 skip 28건은 별도 guard 묶음에서 **66 passed / 28 skipped / exit 0**으로 전부
  `Playwright subprocess skipped in Codex sandbox`임을 직접 확인했다. 기준환경 예상치는
  **1303 passed / 4 skipped**이나 이 환경에서 직접 실행한 값은 아니다.
- 패킷 §6 3파일 묶음:
  `tests/test_golden_sweep.py tests/test_daewoon_qiyun_axis.py tests/test_solar_term_axis.py -q`
  → **136 passed / exit 0 / 1221.56s(20분 21초)**. `_NAMED` 7건·골든 22건·월지 교차검증 GREEN.
- Ruff(변경 Python 5파일) → `All checks passed!` / exit 0.
- `git diff --check` → exit 0(CRLF 안내만). 최종 dirty 경로는 허용된 제품 3파일·테스트 2파일·
  본 보고서뿐이며 forbidden 침범 0이다.

## 미검증·남은 위험·다음 행동

- 기준환경의 예상 `1303/4`는 직접 실행하지 못했다. 동일 수집 총수와 Playwright 28건 skip 사유로
  환경 차이를 분리했으며 코드 실패는 0이다.
- 실 PDF·`hrun`·육안 검수·LLM/API 호출은 패킷 금지/비요구라 실행하지 않았다.
- 격자 옵트인 전환은 성능 권고일 뿐 이번 범위에서 구현하지 않았다. 다음은 Claude Code 신선 세션의
