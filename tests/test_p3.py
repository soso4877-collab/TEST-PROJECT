...
def test_gakron_and_new_sections_present_and_deep():
    ...
    # T0-④: health 는 의료 단정 금지 프레이밍을 유지하되, 병원 안내형 보일러플레이트는 쓰지 않는다.
    ht = by["health"].final_text
    for term in ("병에 걸린", "병이 생긴", "불치", "수명이 짧", "죽는다", "사망", "단명"):
        assert term not in ht, f"health 의료 단정어: {term}"
    assert "병을 진단하는 자리가 아니라" in ht
    assert "몸 상태 변화는 그때그때 기록" in ht
    for hedge in ("전문가와 상의", "참고용", "참고로만", "병원에서 확인해 보세요"):
...
def test_llm_fallback_when_polish_hallucinates(monkeypatch):
    ...
    monkeypatch.setattr(
        llm_polish, "polish", lambda rt, title, **_kw: rt + " 추가로 甲寅 대운이 옵니다."
    )
