...
- 생시 미상(출생 시각 모름): 세 기둥(연·월·일)과 12개 시지 후보에서 전부 같은 사실만 사용한다. 정오·시주·자미는 만들지 않는다(00-immutable §8). 입춘·절입이 신고 날짜 안에 있어 시각 없이 연주·월주를 하나로 정할 수 없으면 추가 정보 요청 상태로 차단한다.
- 생시 미상 v1은 1인 모듈(love/job/wealth/health)만 지원한다. 상대 입력 또는 gunghap은 두 사람 관계 사실의 12후보 불변 축약이 아직 없으므로 접수/모듈 확정 단계에서 차단한다.
...
- 생시 미상 주문은 고객 원국이 3열(연·월·일)인지, 고지 문장이 정확한지, 시주·자미·정오/진태양시 보정 표현이 0인지 확인한다. provenance가 없는 과거 unknown 주문은 발급하지 말고 새 정책으로 재생성한다.
