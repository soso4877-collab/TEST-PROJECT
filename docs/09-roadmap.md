...
| 1 | KASI 검증층: 키 발급(운영자)→실호출 확정→전수 캐싱→3원 교차 | scripts/kasi_dump.py, calc/kasi.py, tests/test_kasi.py | 1900~2050 캐시+불일치 0 또는 전수 문서화 | |
...
| 6 | Admin Review UI | sajugen/admin/, content/repetition.py | 주문 1건 풀사이클 리허설 | |
| 7 | 안전·반복 필터 보강+테스트 100+ | safe_lint 확장, 골든 자동 케이스 | pytest 100+ GREEN, veraPDF 비악화 | |
| 8 | MVP 릴리스 체크리스트 | delete_order.py, README-ops, CLAUDE.md | E2E 3건(평일/윤달/시진불명) PASS | |
