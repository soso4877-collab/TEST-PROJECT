...
- 이월(미해결, 블로커 아님 — 상세는 `REVIEW-FEEDBACK.md` 2026-08-17 F-1~F-3):
  ...
  - **F-2**: `calc/partner.py:154-157`(궁합 상대)은 같은 −14분 결함이 잔존한다(경계 5건 중 2건 오답). 이 교정으로 본인=교정축 / 상대=미교정축이 됐고, docstring 의 "`calc/myeongni.build` 와 동일 경로" 는 **거짓이 됐다**(방법론 A-5). 후속 패킷 1순위.
