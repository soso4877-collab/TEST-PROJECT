...
- 미검증·잔여: 실 PDF·실모델·`hrun`·육안 검수 0건(합성 테스트 산출물 외 PDF 생성 0). 잔여 2건은 이 패킷이 고치지 않고 운영자 확인으로 올린다 — (1) **factcheck 일상어 동형 예외**(`_GANZHI_KO_COMMON_WORDS` = 계신·임신·기사·무사·병사·정사·기미): 억제 대상 연·월주가 이 집합에 들면 allow-set 축소가 무효다. 그래서 결함 주입 케이스는 이 집합 **밖** 간지(`乙丑`·`己丑`)로 골랐다 — 안에서 고르면 방어를 검증하지 못한 채 GREEN 이 된다. (2) **다인 allow-set 공유**: 허용 토큰은 주문 단위·토큰 수준이라 억제된 상대 A 의 연·월주가 같은 주문의 다른 상대 B 의 실제 간지와 겹치면 허용된다(인물별 분리는 구조 변경).
  - **[2026-08-23 N-5 조사 종결 — 운영자 결정 O1(수용+기록), 코드 무변경]** 1960~2030 경계일 852일 전수 프로브 실측으로 위 두 잔여의 폭을 정정한다. (1) 억제 대상 연·월주가 일상어 동형 집합에 드는 경계일 = **98/852 = 11.5%**(60갑자에 실재하는 동형어는 壬申·己巳·丁巳·己未 4개뿐. 계신·무사·병사는 간지가 아님). 예외를 접미 조건부로 좁히는 개선은 `임신 생각`·`기사 일을`·`무사 일 년` 이 전부 간지로 매칭돼(정규식 `\s*`+`일|시|생` 접미) 오탐 재발이 실측됐으므로 채택하지 않는다. (2) "다인 공유"는 오진단이다 — 실체는 **토큰 수준 factcheck 의 귀속(attribution) 무지**로, 상대 1인만 있어도 억제 연·월주가 본인 허용집합(4주·대운·세운·월운 = 22~29/60)에 이미 들어 있는 경계일이 합성 본인 4명 기준 **63.6~74.2%** 다. 즉 2차 벨트(factcheck)는 경계일 억제의 약 2/3 에서 원리적으로 no-op 이며, 1차 방어(`rules.partner_block` 문안 제외 + 프롬프트 일주만 제공)가 실제 방어다. 한자 표기는 계속 차단된다. 억제 deny-set(O2)은 같은 65% 겹침에서 본인 정당 언급까지 오차단해 기각, 인물별 귀속 파서(O3)는 방법론 B-8 어설픈 게이트로 기각. 잔여 위협 = 본인 문맥 토큰의 상대 오귀속인데 이는 factcheck 전체의 기존 한계이지 N-5 고유 신규 노출이 아니다. 미검증: 실 LLM 되살림 빈도(유료 실측 0).
...
- 이월(블로커 아님 — 상세는 `REVIEW-FEEDBACK.md` 2026-08-17 F-1~F-3. **F-2 는 2026-08-18 해소, F-1 은 미해결**):
  - **F-1(교차리뷰 신규 발견)**: 대운 `start_year` 가 균일 무작위 3,000건에서 **0.53~0.83%** 이동한다. `getSolar`가 절대축이라 `DaYun` 의 달력 앵커가 CST 프레임 날짜가 되기 때문이고, 起運 **거리**를 절대축으로 둔 결정과 달리 **달력 연도 환산 앵커**는 패킷이 판단하지 않았다. 전파는 한정적(동일연도 세운 간지 충돌 **0/3000**, 월운 **0/3000**, 세운 연도집합 0.100%). 기존 가드 `test_golden_sweep.py:221` 은 허용폭 ±2라 ±1 이동을 통과시킨다 → **회귀 핀 추가 + 앵커 축 유파 판단 필요**.
    - **[2026-08-21 정정 — 교차검토 반증까지 반영한 최종본]** 이 항목은 하루에 세 번 손댔다: `27464c3`(원인 전복) → `488093b`(감지구멍 재정정) → **별도 세션 read-only 교차검토(Codex)가 두 커밋의 핵심 주장을 반증**했고, 그 반증을 Claude 가 전부 재현·확인한 결과가 아래다. 위 원 진단은 폐기가 아니라 **부분 원인으로 격하**한다.
      **주의 — `27464c3`·`488093b` 의 커밋 메시지 본문은 폐기된 주장을 담고 있다**("항상 함께 밀려 허용폭과 무관하게 통과" · "골든은 눈이 멀지 않았다" · "0/7·3/7" · "새 수치 생성 0 = 전부 §3 인용" — 실제로 0/3000·0.100% 는 §3 이 아니라 `REVIEW-FEEDBACK.md` 출처). **`git log` 를 F-1 근거로 쓰지 말 것. 이 블록이 정본이다.**
      - **원인(유지, 단 "진범" 표현은 철회)**: `getSolar()` 는 `Yun`/`DaYun` 에서 세 역할을 겸직한다 — (a) 절입까지의 **거리** 끝점 / (b) **시진 버킷팅**(`Yun.py:39-52` 의 `getTimeZhiIndex`) / (c) 起運 날짜의 **달력 앵커**·`birth_year`. 관측 차이의 **지배적 메커니즘은 (b)** 이고 (c) 는 00시대 전용이다(CST=KST−1h 라 홀수시만 시진을 건너뛴다). **단 (b) 의 올바른 축이 미결정이므로 "오류" 로 확정하지 않는다** — 아래 '축 근거' 참조.
      - **층화 실측(2026-08-21 재현 완료)** — **동일** 격자(1985~1994·연도별 리셋 7일 간격·전 시각·남녀·분30): 홀수시 **321/12,720 = 2.52%** · 00시대 **32/1,060 = 3.02%** · 짝수시(0시 제외) **8/11,660 = 0.069%** · 전체 **361/25,440 = 1.42%**. 00시대를 전부 (c) 수정으로 제거한다고 과대 가정해도 **329/361 = 91.1% 잔존** → **(c) 단독 수정은 불충분**(이 정성 결론만 유지). ~~"약 87% 잔존" · "(b)1.0%p+(c)0.12%p 가 1.42% 와 정합"~~ 은 **폐기** — 서로 다른 표본 설계(기간·간격)의 비율을 합성한 값이라 재현되지 않는다. 인과 기여도는 **한 축만 바꾸는 반사실 프로브**로 다시 재야 한다. 위 `0.53~0.83%`(균일 무작위 3,000건)와도 여전히 합치지 않는다.
      - **폭 분포**(부록 A-2 방식 = **연속** 7일 격자 **25,056건**. A-1 의 25,440건과 **다른 표본**): 불일치 **371건** = ±1년 **337(90.8%)** + 꼬리 |Δ|≥9 **32건(8.6%)** + 0년 2건. **`docs/26` §3-5 의 "30건 / 91.8%" 는 오기** — 재현값은 32건 / 90.8% 다.
      - **심각도 — 상향 판정을 철회한다**(2026-08-21 재측정). `current_daewoon()` 간지가 실제로 갈리는 건수를 **불일치 371건 전수**에 대해 쟀다: ref 2026 **36건**(전체 표본 대비 **0.14%**), ref 2010·2020·2030 각 34건, ref 2000 47건. **그중 꼬리 32건의 기여는 ref 2026 에서 2건뿐이고 ref 2010·2020·2030 에서는 0** 이며, 나머지 34건은 평범한 **±1년 이동**이다. 즉 **"±10년 꼬리가 질적으로 다른 실패 모드"라는 서술을 실측이 지지하지 않는다** — 꼬리는 간지열이 한 슬롯 정렬돼 대체로 상쇄된다. 2026-06-14 "대운 모순(정미/병오 혼서)" 과 **동계열이라는 단정도 철회**한다. 원 문장의 전파 측정치(0/3000·0.100%)는 `REVIEW-FEEDBACK.md` 선행 보고치로 **보존**하되, 재현 스크립트·seed 가 저장되지 않아 이번에 **독립 재검증하지 못했다**.
      - **감지 구멍**: `:221`(`test_daewoon_structure_invariants`)은 축 간 기대값을 비교하지 않는 **구조 불변식**이라 대부분의 프레임 드리프트를 놓친다 — 여기까지는 유효. 그러나 ~~"`start_year` 와 `start_age` 가 항상 함께 밀려 허용폭과 무관하게 통과한다"~~ 는 **거짓**이다. 재현: `1985-03-05 23:30 남` → Δqiyun **+9** / Δstart_year **+10**(서로 다름), `1988-01-01 00:30 남` → CST 프레임 잔차 **−1 로 `:221` 실제 실패**. 검출력이 매우 약할 뿐 0 은 아니다. 또 ~~"골든은 눈이 멀지 않았다"~~ 도 **철회** — `_NAMED` 대운수 7개는 `d3b5972`("골든 회귀 확장 … **손대조 0**")에서 구현과 **동시 도입된 스냅샷**이고, `docs/12` §6 이 열거한 독립 오라클(4기둥·방향·공망)에 **대운수 절대값은 없다** → 정확성 오라클로 쓰지 않는다. 남는 진짜 구멍 = **축 간 대조 부재 + 자연 잔차 0~2 허용 + 대운수 절대 오라클 부재 + 날짜·시진 경계 표본 부재**의 조합. (부수 유지) **층화하지 않은 무작위 표본이 홀/짝 비대칭을 평균으로 지웠다** — 원 진단이 (b) 를 못 보고 (c) 로 귀결된 이유가 이것이다.
      - **축 민감도 — `0/7 · 3/7` 은 철회**. 그건 `time_correction` 의 **시지** 차이일 뿐 대운수 민감도가 아니었다. 재측정: `_NAMED` 7건의 `(qiyun, start_year)` 는 CST·시민KST·진태양시 **세 축에서 전부 동일 = 0/7 · 0/7**. 이유는 `Yun.__compute_start` 가 시진뿐 아니라 `day_diff = end.subtract(start)`(**달력 일수**)도 쓰기 때문 — **시진 일치가 대운수 일치를 함의하지 않고 그 역도 아니다.** 게다가 `Yun` 은 `hour == 23 → 亥` 특례를 쓰는데 `LunarUtil.getTimeZhiIndex` 는 같은 시각을 **子** 로 준다(세 구현의 23시 경계가 서로 다르다).
      - **축 근거 — `docs/03` 신규 확정 행 등재 보류**. "고전의 시진 = 진태양시" 를 뒷받침하는 1차 자료가 저장소에 없다(`docs/00-research-ledger.md` 관련어 검색 0건). 기존 일주·시주와의 축 일관성은 **구현상 장점일 뿐 유파 근거가 아니다.** 1차 문헌 검증 + 운영자 결정 전까지 **미결정**으로 둔다.
      - **처방(방향은 유지)**: 프록시 축 재배정으로는 못 고친다(한 메서드를 두 축에 동시 배정 불가). 起運 계산을 lunar-python 에서 회수 — 거리=Skyfield `solar_term_time`(초 단위), 이산화=**결정된 축**, 앵커=시민 KST. 다음 패킷은 **유파 1차 자료 조사 + 축별 반사실 프로브**(한 축만 바꿔 기여도를 분리)를 포함해야 한다. 재현 스크립트 전문은 `docs/26` 부록 A.
