...
- 이월(블로커 아님 — 상세는 `REVIEW-FEEDBACK.md` 2026-08-17 F-1~F-3. **F-2 는 2026-08-18 해소, F-1 은 미해결**):
  - **F-1(교차리뷰 신규 발견)**: 대운 `start_year` 가 균일 무작위 3,000건에서 **0.53~0.83%** 이동한다. `getSolar`가 절대축이라 `DaYun` 의 달력 앵커가 CST 프레임 날짜가 되기 때문이고, 起運 **거리**를 절대축으로 둔 결정과 달리 **달력 연도 환산 앵커**는 패킷이 판단하지 않았다. 전파는 한정적(동일연도 세운 간지 충돌 **0/3000**, 월운 **0/3000**, 세운 연도집합 0.100%). 기존 가드 `test_golden_sweep.py:221` 은 허용폭 ±2라 ±1 이동을 통과시킨다 → **회귀 핀 추가 + 앵커 축 유파 판단 필요**.
    - **[2026-08-21 정정 — 진범은 달력 앵커가 아니라 시진(時辰) 버킷팅이다]** 근거 = `docs/26-engine-eval-2026-08-20.md` §3(lunar-python 1.4.8 격리 환경 독립 재현). **위 원 진단은 폐기가 아니라 부분 원인으로 격하한다.**
      ...
      - **감지 구멍(2층)**: (1) ~~`test_golden_sweep.py:221` 의 허용폭 ±2 로는 ±10년 꼬리를 못 잡는다~~ → **[2026-08-21 재정정 — 같은 날 자기 교정, 감지구멍을 잘못 지목했었다]** 이건 **허용폭 문제가 아니다.** `test_daewoon_structure_invariants:221` 은 `0 <= (start_year − 출생연) − start_age <= 2` 라는 **자기정합 불변식**이라, 프레임이 통째로 밀리면 `start_year` 와 `start_age`(= `daewoon_count` + 10i)가 **함께** 밀려 허용폭과 무관하게 통과한다 — 애초에 축 오류를 검출하는 단언이 아니다. 그리고 골든은 눈이 멀지 않았다: `test_named_cases:344` 가 `daewoon_count` **절대값을 7건 핀**한다. 진짜 구멍은 **그 7건이 전부 (b) 비영향 구간에 있다는 커버리지 갭**이다 — 실측(2026-08-21, `input.time_correction` 기준 시진 대조): 현행 CST 축 ↔ 진태양시 축에서 시진이 갈리는 골든 케이스 **0/7**. 그래서 (b)가 골든에 한 번도 나타나지 않았다. 시사점 = 회귀 핀은 허용폭을 조이는 게 아니라 **종류가 다른 단언**(프레임 간 대조 또는 절대 `start_year` 핀)이어야 하고, 케이스를 **(b) 영향 구간에서 골라야** 한다. 참고로 축 B(시민 KST)를 택하면 같은 골든 7건 중 **3/7** 에서 시진이 갈린다(재도출 필요). `docs/26` §3-5·§3-7 의 "±2 허용폭" 서술도 같은 이유로 부정확하다. (2) **층화하지 않은 무작위 표본이 홀/짝 비대칭을 평균으로 지웠다** — 원 진단이 (b) 를 못 보고 (c) 로 귀결된 이유가 이것이다. 이 사고의 교훈("좁은 경계는 랜덤 표본으로 안 잡힌다")이 **원인 진단 단계에서 한 번 더 반복된 셈**이다.
